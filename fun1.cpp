...
bool check_Csv_File_Extention(std::string file_Name){
  ...
  if(l>4)
  {
    ...
    {
    std::cout << "----------------------------------\n";	
    std::cout << "--   ERROR: Invalid Extention   --\n";
    std::cout << "----------------------------------\n";
    return 1; 
    }
  }
...
std::string get_Csv_File_Name() {
  ...
  while(is_Csv){
  std::cout << "\n***   Input CSV File Name along with extention   ***\n";
  std::cin >> file_Name;
...
void display_File() {
  system("./set_Permission.sh");
  system("./display_File.sh");
}
...
int set_Target_Col(int total_Cols) {
  bool is_Not_Valid_Target=1;
  int target;
  std::string line2;
  int count_Decimal=0;
  while(is_Not_Valid_Target){
  std::cout << "***   Enter the target Column by Column number   ***\n";
  std::cin >> line2;
  count_Decimal=0;
  bool temp=0;
  is_Not_Valid_Target=check_Number(line2);
  is_Not_Valid_Target=!is_Not_Valid_Target;
  if(is_Not_Valid_Target==0)
    {
      target=stoi(line2);
      if(target<=total_Cols&&target>0)
      is_Not_Valid_Target=0;
      else
      {
      	std::cout<<"------------------------------------------\n";
        std::cout<<"      ERROR: Target Not in Range\n";
        std::cout<<"  Target range should be between 1 and "<<total_Cols<<"\n";
        std::cout<<"------------------------------------------\n";
        is_Not_Valid_Target=1;
      }
    }
  else
  {
...
bool check_Labels(std::string file_Name)
{
  ...
  for (int i = 0; i < line.length()+4; ++i)
  std::cout<<"-";
  std::cout<<"\n***   Press 1 if the above Records are the labels   ***\n";
  std::cout<<"***   Press 0 if the above Records are not labels   ***\n";
  std::string is_Label="";
  std::cin>>is_Label;
  if(is_Label.length()>2)
    ...
    if(is_Valid==1)
    {
      ...
      std::cout<<"  NOTE: Enter Value either 1 or 0  \n";
      std::cout<<"-----------------------------------\n";
    }
  };
  return label_Exists;
...
bool check_Dimensions(int total_Rows,int total_Cols,std::string file_Name)
{
	  if(total_Cols==1||total_Rows==-1)
	  	{
	  		std::cout<<"---------------------------\n";
	  ...
	  {
	  	std::cout<<"***   Dimension of the given file \""<<file_Name<<"\" is "<<total_Rows<<" X "<<total_Cols<<"   ***\n\n";
	  	return 0;
	  }
}
    ...
    if (mapper.find(line) == mapper.end()) 
    {
        while(is_Not_Integer)
        {
          ...
          for(int j=0;j<line2.length();j++)
          {
            if(line2[j]=='.')
            {
            ...
            else 
              {
                is_Not_Integer=1;
                std::cout<<"---------------------------------------------\n";
                std::cout<<"  ERROR: Enter a numeric value not a String  \n";
                std::cout<<"---------------------------------------------\n";
                break;
              }
          }
          if(is_Not_Integer==0&&count_Decimal<2)
            ...
            {
              std::cout<<"-------------------------------\n";
              std::cout<<"  ERROR: Invalid Numeric Data  \n";
              std::cout<<"-------------------------------\n";
              is_Not_Integer=1;
            }
        };
...
void check_Nominal_Ordinal(std::vector < std::string > * data,
  ...
  for (int i = 0; i < remaining_Cols; i++) {
    if (col_Data_Type[i] != 1) {
      std::cout << "***   Does the Column " << i + 1 << " have an order   ***\n";
      std::cout<<"***   Sample of Column " << i + 1 << "   ****\n";
      ...
      {
      	std::cout<<"-----------------------------------\n";
        std::cout<<"  ERROR: Entered value is invalid  \n";
      	std::cout<<"-----------------------------------\n";
      }
...
void classification(int total_Cols,int total_Rows)
{
  ...
  bool is_Valid=1;
   ...
   // std::cout<<"***     1: Logistic Regression               ***\n";
...
void clustering(int total_Cols,int total_Rows)
{
  ...
  bool is_Valid=1;
    ...
    std::cout<<"***     2: Mean Shift Clustering         ***\n";
    std::cin>>line2;
...
void regression(int total_Cols,int total_Rows)
{
  ...
  bool is_Valid=1;
    ...
    std::cout<<"***     2: Least Angle Regression        ***\n";
    std::cin>>line2;
    if(line2.length()==1&&(line2[0]=='1'||line2[0]=='2'))
      is_Valid=0;
    else
...
void machineLearning(int total_Cols,int total_Rows)
{
  ...
  bool is_Valid=1;
    ...
    std::cout<<"***      2: Classification           ***\n";
    std::cout<<"***      3: Clustering               ***\n";
    std::cout<<"****************************************\n";
    std::cin>>line2;
    if(line2.length()==1&&(line2[0]=='1'||line2[0]=='2'||line2[0]=='3'))
      is_Valid=0;
  ...
  switch(val)
  {
    ...
    std::cout<<"***   Created by Utkarsh Darbari (Manipal University Jaipur, B.TECH I.T 8-A, 159102149)   ***\n";
    std::cout<<"*********************************************************************************************\n\n";
}
...
int main() {
  //FUNCTION 1
  bool is_Not_Valid_Filename=1;
  int total_Cols=0,total_Rows=0;
  std::string file_Name="";
	while(is_Not_Valid_Filename)
	{ 
	  file_Name = get_Csv_File_Name();
	  store_To_File(file_Name);
	  total_Rows=get_Total_Rows(file_Name);
	  total_Cols=get_Total_Cols(file_Name);
	  is_Not_Valid_Filename=check_Dimensions(total_Rows,total_Cols,file_Name);
	};
  display_File();
  //FUNCTION 2
  show_Labels(file_Name,total_Cols);
  int target_Col=set_Target_Col(total_Cols);
  std::cout<<"---   Target column set as "<<target_Col<<"   ---\n";
  //FUNCTION 3
  std::set <int> remove_Col_Idx;
